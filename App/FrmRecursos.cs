    ...
    public partial class FrmRecursos : Form
    {
        ...
        private void editarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.dataGridView1.SelectedRows.Count > 0)
                {
                    int id = int.Parse(this.dataGridView1.SelectedRows[0].Cells["IdRecurso"].Value.ToString());  // Obtiene el 'IdRecurso' de la fila seleccionada.
                    FrmAddRecursos frm = new FrmAddRecursos(id); // Crea una nueva instancia del formulario FrmAddRecursos, pasando el 'id' del recurso seleccionado
                    this.Close(); // Cierra el formulario actual
                    frm.Show(); // Muestra el formulario de edición del recurso
                }
            }
            catch (Exception ex)
            {
